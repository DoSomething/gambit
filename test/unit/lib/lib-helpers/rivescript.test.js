...
test('getTriggerRivescriptLine should return trigger command with triggerText as rs line', () => {
  sandbox.stub(rivescriptHelper, 'formatRivescriptLine')
    .returns(mockRivescriptLine);

  const result = rivescriptHelper.getTriggerRivescriptLine(mockWord);
...
test('getRivescriptFromDefaultTopicTrigger returns replyRivescript if defaultTopicTrigger.redirect is not set', () => {
  ...
  rivescriptHelper.getReplyRivescript.should.have.been.calledWith(replyDefaultTopicTrigger);
  result.should.equal(mockRivescript);
});
