...
function executeMacro(macroName, user) {
  if (macroName === 'post_signup') {
    return user.postSignup().then(() => helpers.getSignupConfirmedMessage(campaign));
  }
  if (macroName === 'decline_signup') {
    return user.declineSignup().then(() => helpers.getSignupDeclinedMessage(campaign));
  }
  
  // If we've made it this far, we're matching the catchall star trigger in /brain/star.rive.

  if (user.topic === 'michael') {
    return helpers.getInvalidMichaelMessage();
  }

  // If we're in any other topic, bring it back to a Campaign.
  if (!user.topic || user.topic.indexOf('campaign') < 0) {
    return user.promptSignupForCampaignId(campaign.id)
      .then(() => helpers.getSignupPromptMessage(campaign));
  }

  return helpers.getSignupConfirmedMessage(campaign);
}
