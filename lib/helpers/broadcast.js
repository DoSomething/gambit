...
async function fetchById(id) {
  ...
  const hasTopic = broadcast.saidYesTopic || broadcast.topic;

  if (hasTopic) {
    const topicProperty = broadcast.saidYesTopic ? 'saidYesTopic' : 'topic';

    // TODO: Refactor codebase to check for the topic GraphQL __typename, not the Contentful
    // type name
    Object.assign(broadcast[topicProperty], {
      type: broadcast.contentType,
      /**
       * Adding broadcastId Fixes bug that would save the topic id as the broadcast id in
       * conversation.lastReceivedBroadcastId if the topic is broadcastable
       */
