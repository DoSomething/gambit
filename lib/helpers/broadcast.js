...
module.exports = {
  ...
  getMessageCount: function getMessageCount(broadcastId, direction) {
    const key = `${broadcastId}-${direction}`;
    const query = { broadcastId, direction };
    return statsCache.get(key)
      .then((cachedResult) => {
        ...
        return Message.where(query).count()
          .then(count => statsCache.set(key, count));
      });
  },
  getTotalInboundMessagesForBroadcastId(broadcastId) {
    return exports.getMessageCount(broadcastId, 'inbound');
  },
  getTotalOutboundMessagesForBroadcastId(broadcastId) {
    return exports.getMessageCount(broadcastId, 'outbound-api-import');
  },
};
