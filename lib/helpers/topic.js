...
async function fetchById(id) {
  ...
  const data = Object.assign(topic, { type: topic.contentType });
  return helpers.cache.contentfulEntries.set(id, data);
}
...
async function getById(id) {
  ...
  const topic = await helpers.cache.contentfulEntries.get(id);
  if (topic) {
