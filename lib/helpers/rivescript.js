...
function fetchRivescripts() {
  return graphql.fetchConversationTriggers({ cache: false, limit: 150 })
    // TODO: Check our res.meta to determine whether to fetch more triggers.
    .then((conversationTriggers) => {
      logger.debug('fetchDefaultTopicTriggers success', { count: conversationTriggers.length });
      return conversationTriggers.map(module.exports.parseRivescript);
    })
...
function parseRivescript(defaultTopicTrigger) {
  ...
  /**
   * The next line is a reply command, e.g.
