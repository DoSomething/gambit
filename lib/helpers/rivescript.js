...
function getBotReply(userId, topicId, messageText) {
  // When the Rivescript bot doesn't have a topic definition loaded for a given topicId, it outputs
  // a "User x was in an empty topic Y" message to the console. This avoids that message.
  const rivescriptTopicName = helpers.topic.isHardcodedTopicId(topicId) ? topicId : 'random';
  return rivescript.getBotReply(userId, rivescriptTopicName, messageText);
}
