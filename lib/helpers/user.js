...
async function updateByMemberMessageReq(req) {
  ...
  // If we need to update a member's property based on a macro, we add the field and value here
  underscore.extend(payload, helpers.macro.getProfileUpdate(req.macro));
  // Backfill the member's address reported by Twilio
  // Maybe we should "update" this info if Twilio reports an updated address?
  ...
  // The voting plan logic should be refactored if we want to keep using it for future elections.
  // TODO: Move this side effect out as part of refactoring the voting plan feature
...
/**
 * sets req.suppressOutbound to true if user has been and continues to be unsubscribed
 * @param {Request} req
 * @param {User}    user
 * @param {User}    updatedUser
 * @returns {Boolean}
 */
function suppressReplyIfUnsubscribed(req, user, updatedUser) {
  const wasSubscriber = module.exports.isSubscriber(user);
  const isSubscriber = module.exports.isSubscriber(updatedUser);
  if (!wasSubscriber && !isSubscriber) {
    helpers.request.setSuppressOutbound(req);
    // Exposed for monitoring
    logger.info('suppressReplyIfUnsubscribed(): suppressing reply for unsubscribed member', {
      id: updatedUser.id,
      wasSubscriber,
      isSubscriber,
    }, req);
