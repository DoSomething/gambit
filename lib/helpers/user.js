...
async function createPhotoPost({
  ...
  payload[
    gatewayClientConfig.photoPostCreation.fileProperty
  ] = await helpers.util.fetchImageFileFromUrl(photoPostValues.url);
 ...
 * @return {Promise}
 */
async function fetchOrCreateSignup(
  user,
  campaign,
  signupSource,
  signupSourceDetails,
  ...
  return module.exports.createSignup(
    user,
    campaign,
    signupSource,
    signupSourceDetails,
...
async function fetchSignup(user, campaign) {
  const res = await gateway.fetchSignups(
    module.exports.getFetchSignupsQuery(user.id, campaign.id),
  );
...
async function fetchVotingPlan(user) {
  const res = await gateway.fetchPosts(
    module.exports.getFetchVotingPlanQuery(user.id),
  );
  return res.data && res.data[0] ? res.data[0] : null;
}
...
async function updateByMemberMessageReq(req) {
  ...
  if (helpers.macro.isCompletedVotingPlan(req.macro)) {
    const votingPlan = await module.exports.fetchOrCreateVotingPlan(
      user,
      req.platform,
      req.platformUserStateISOCode,
    );
