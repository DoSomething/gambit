...
async function createSignup(user, campaign, signupSource, signupSourceDetails) {
  const payload = {
    ...
    // TODO: When Rogue campaigns go live, we can remove this field completely.
 ...
 * @return {Promise}
 */
async function createPhotoPost(user, campaign, photoPostSource, photoPostValues) {
  const payload = {
    northstar_id: user.id,
    campaign_id: campaign.id,
    ...
    // TODO: Remove this field once Rogue campaigns are imported.
    campaign_run_id: campaign.currentCampaignRun.id,
  };
  ...
  payload[rogueClientConfig.photoPostCreation.fileProperty] = await helpers.util
 ...
 * @return {Promise}
 */
async function createTextPost(user, campaign, textPostSource, textPostText) {
  const payload = {
    northstar_id: user.id,
    campaign_id: campaign.id,
 ...
 * @return {Promise}
 */
async function fetchOrCreateSignup(user, campaign, source, details) {
  ...
  return module.exports.createSignup(user, campaign, source, details);
}
...
async function fetchSignup(user, campaign) {
  // TODO: Use campaign.id after Rogue campaigns go live.
  const campaignId = campaign.currentCampaignRun.id;
