...
function getRivescriptReply(req) {
  return helpers.rivescript.getBotReply(req.userId, req.conversation.topic, req.inboundMessageText);
}
...
function parseAskYesNoResponse(req) {
  return helpers.rivescript.parseAskYesNoResponse(req.inboundMessageText)
    .then((result) => {
      if (helpers.macro.isSaidYes(result) || helpers.macro.isSaidNo(result)) {
        req.macro = result;
      }
    });
}
