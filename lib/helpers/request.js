 ...
 * @return {Promise}
 */
async function updateTopicIfChanged(req, topic) {
  if (req.currentTopicId !== topic.id) {
    module.exports.setTopic(req, topic);
    await req.conversation.setTopic(topic);
  }
  /**
   * We could send members different broadcasts, with different ids
   * that share the same topic id. For this reason, we would need to
   * update the lastReceivedBroadcastId regardless of if the topic ids
   * remain the same.
   */
  if (helpers.topic.isBroadcastable(topic)) {
    let campaignId = null;
    let broadcastId = null;
    // AskYesNo broadcast topics have the campaign metadata inside saidYesTopic
    const nestedTopic = topic.saidYesTopic || topic.topic;

    if (nestedTopic) {
      broadcastId = nestedTopic.broadcastId;
      campaignId = nestedTopic.campaign ? nestedTopic.campaign.id : null;
    } else {
      // AskSubscriptionStatus would not have a nested topic so the broadcastId
      // is the topic.id
      broadcastId = topic.broadcastId || topic.id;
      campaignId = topic.campaign ? topic.campaign.id : null;
    }

    await req.conversation.setLastReceivedBroadcast(broadcastId, campaignId);
  }
}
...
function setTopic(req, topic) {
  ...
  /**
   * FIXME: in the case of an askYesNo broadcast, because the topic here is the whole
   * broadcast entry and the campaign info is inside the `saidYesTopic` nested entry.
   * We do not set the campaign.
   */
  if (topic.campaign) {
