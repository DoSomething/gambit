...
function changeTopicByCampaign(req, campaign) {
  ...
  // Broadcasts will eventually be topic specific instead of campaign specific, so we won't
  // be calling this function from there.
  if (!campaign.topics.length) {
    return Promise.reject(new Error('Campaign does not have any topics'));
  }
  return module.exports.changeTopic(req, campaign.topics[0]);
}
