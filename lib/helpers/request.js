...
async function updateTopicIfChanged(req, topic) {
  ...
  if (helpers.topic.isBroadcastable(topic)) {
    ...
    const hasTopic = topic.saidYesTopic || topic.topic;

    if (hasTopic) {
      // AskYesNo broadcasts
      if (topic.saidYesTopic) {
        broadcastId = topic.saidYesTopic.broadcastId;
        campaignId = topic.saidYesTopic.campaign ? topic.saidYesTopic.campaign.id : null;
      } else {
        broadcastId = topic.broadcastId;
        campaignId = topic.campaign ? topic.campaign.id : null;
      }
    } else {
      broadcastId = topic.id;
    }
...
function setTopic(req, topic) {
  ...
  /**
   * TODO:/FIXME: in the case of an askYesNo broadcast, because the topic here is the whole
