...
async function updateTopicIfChanged(req, topic) {
  ...
  if (helpers.topic.isBroadcastable(topic)) {
    const campaignId = topic.campaign ? topic.campaign.id : null;
    const lastReceivedBroadcastId = req.conversation.lastReceivedBroadcastId;
    if (lastReceivedBroadcastId !== topic.broadcastId) {
      await req.conversation.setLastReceivedBroadcast(topic.broadcastId, campaignId);
    }
  }
}
