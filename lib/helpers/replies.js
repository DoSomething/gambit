...
module.exports.sendReply = async function (req, res, messageText, messageTemplate) {
  ...
  const wasSubscribed = helpers.user.isSubscriber(req.user);
  logger.debug('sendReply', { messageTemplate }, req);

  try {
    const user = await helpers.user.updateByMemberMessageReq(req);
    const isSubscribed = helpers.user.isSubscriber(user);
    ...
    // Suppress if member was unsubscribed before this request and it's not resubscribing now
