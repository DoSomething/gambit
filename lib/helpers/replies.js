...
module.exports.sendReply = async function (req, res, messageText, messageTemplate) {
  ...
  try {
    ...
    // Suppresses reply if member was unsubscribed before this request
    // and it's not resubscribing now
    helpers.user.suppressReplyIfUnsubscribed(req, req.user, user);
    // set updated user
