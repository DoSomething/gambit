...
module.exports.sendResponseWithStatusCode = function (res, code = 200, message = 'OK') {
  ...
  if (code < 300) {
    logger.debug('sendResponseWithStatusCode',
      helpers.request.injectRequestId({ code, response }, res));
  } else {
    logger.error('sendResponseWithStatusCode',
      helpers.request.injectRequestId({ code, response }, res));
  }
...
module.exports.sendResponseWithMessage = function (res, message) {
  logger.debug('sendResponseWithMessage',
    helpers.request.injectRequestId({ messageId: message.id }, res));
...
function sendReply(req, res, messageText, messageTemplate) {
  logger.debug('sendReply',
    helpers.request.injectRequestId({ messageText, messageTemplate }, req));
  ...
  return gambitCampaigns.postReceiveMessage(req)
    .then((gambitCampaignsRes) => {
      ...
      logger.debug('continueCampaign',
        helpers.request.injectRequestId({ signupId: req.signup.id }, req));
...
module.exports.addBlinkSuppressHeaders = function addBlinkSuppressHeaders(res) {
  logger.trace('Adding Blink supress headers', helpers.request.injectRequestId(res));
