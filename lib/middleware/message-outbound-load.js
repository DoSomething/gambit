...
module.exports = function loadOutboundMessage(config) {
  return (req, res, next) => {
    ...
    logger.debug('loadOutboundMessage: Is a retry request. Proceeding with loading message.',
      helpers.request.injectRequestId(config, req));
    logger.debug('loadOutboundMessage: metadata.',
      helpers.request.injectRequestId(req.metadata, req));
    ...
    return Message.updateMessageByRequestIdAndDirection(req.metadata.requestId, update,
      ...
      .then((message) => {
        ...
        if (!message) {
          logger.debug('loadOutboundMessage: Message not found. Calling next to attempt to create it.',
            helpers.request.injectRequestId(config, req));
          return next();
        }
