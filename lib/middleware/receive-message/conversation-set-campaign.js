'use strict';

module.exports = function setConversationCampaign() {
  return (req, res, next) => {

    // If we haven't set a keyword already and this isn't the confirmedCampaign macro, no need to
    // update the User's campaign.
    const updateCampaign = (req.keyword || req.rivescriptReplyText === 'confirmedCampaign');
    if (!updateCampaign) {
      return next();
    }

    let source = 'keyword';
    if (!req.keyword) {
      source = 'menu';
      // Pass the Campaign keyword to Gambit to change the Gambit User Campaign.
      req.keyword = req.campaign.keywords[0];
    }

    req.conversation.setCampaign(req.campaign, source, req.keyword);

    return next();
  };
};
