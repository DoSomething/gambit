  ...
  return async (req, res, next) => {
    try {
      ...
      if (!draftSubmission.values.quantity) {
        ...
        if (quantity) {
          await draftSubmission.addValues({ quantity });
      ...
      if (!draftSubmission.values.url) {
        if (req.mediaUrl) {
          await draftSubmission.addValues({ url: req.mediaUrl });
          return await helpers.replies.askWhyParticipated(req, res);
        }
        return await helpers.replies.invalidPhoto(req, res);
      }

      if (!draftSubmission.values.why_participated) {
        if (inboundMessageText) {
          await draftSubmission.addValues({ whyParticipated: inboundMessageText });
