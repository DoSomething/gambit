...
module.exports = function draftWhyParticipated() {
  return async (req, res, next) => {
    try {
      ...
      const signup = await helpers.user.fetchSignup(req.user, req.topic.campaign.id);
      logger.debug('fetchSignup', { signup }, req);

      // If property exists on signup already, call next to create a new photo post from draft.
      if (signup.why_participated) {
        logger.debug('signup why_participated exists', { signupId: signup.id }, req);
        return next();
      }
      ...
      /**
       * If we've just saved the caption, ask user for they why participated.
       * @see lib/messages/member/topics/photo/draft-caption
       */
      if (req.savedPhotoPostCaption === true) {
