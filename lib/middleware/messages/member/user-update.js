...
module.exports = function updateNorthstarUser() {
  return (req, res, next) => {
    // TODO: When would req.user ever not be set at this point?
    // This might be a MVP relic from before all platforms authenticated with Northstar.
    if (!req.user) {
      return next();
    }
    ...
    try {
      ...
      statusUpdate = helpers.user.parseSubscriptionStatusMacro(req.user, replyText);
      if (statusUpdate) {
