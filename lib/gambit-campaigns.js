 ...
 * @return {object}
 */
function getRandomElementNotEqualToId(array, id) {
  let randomElement = underscore.sample(array);
  while (randomElement.id === id) {
 ...
 * @return {Promise}
 */
module.exports.getRandomCampaignNotEqualTo = function (campaignId) {
  logger.debug('getRandomCampaignNotEqualTo', { campaignId });

  return getActiveCampaigns()
    .then((activeCampaigns) => {
      const randomCampaign = getRandomElementNotEqualToId(activeCampaigns.campaigns, campaignId);

      return exports.getCampaignById(randomCampaign.id);
    });
};
