...
module.exports.getRandomActiveCampaignNotEqualTo = function (campaignId) {
  ...
  // Hardcoding these for now. The GET /campaigns endpoint in Content API is still querying
  // to find all campaigns that contain keywords, which we're looking to soon deorecate.

  const currentActiveCampaignIds = [7, 7059, 7951, 8017, 8064];
  const menuOptions = currentActiveCampaignIds.filter(item => item !== campaignId);
  return exports.getCampaignById(underscore.sample(menuOptions));
};
