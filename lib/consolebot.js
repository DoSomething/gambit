...
const failureInjectionTestHeader = 'x-failure-injection-test';
...
class Consolebot {
  ...
  parseInput(string) {
    ...
    if (string === 'failureTestPass' && this.history.length) {
      const retryPayload = this.history.pop();
      retryPayload.headers[retryHeader] += 1;
      delete retryPayload.headers[failureInjectionTestHeader];
      return retryPayload;
    }
    if (string.indexOf('failureTest') >= 0) {
      const payload = { headers: {} };
      payload.Body = string.replace('failureTest', '');
      payload.headers[retryHeader] = 0;
      payload.headers[failureInjectionTestHeader] = uuidv4();
      return payload;
    }
    return {
      Body: string,
